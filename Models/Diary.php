<?php namespace Foostart\Diary\Models;

use Foostart\Category\Library\Models\FooModel;
use Illuminate\Database\Eloquent\Model;
use Foostart\Comment\Models\Comment;

class Diary extends FooModel {
    ...
    public function getComments($diary_id) {

        // Get diary
        $params = array(
            'id' => $diary_id,
        );
        $diary = $this->selectItem($params);
        ...
        $diary->cache_comments = json_encode($users_comments);
        $diary->cache_time = time();
        $diary->save();
            ...
            foreach($params as $column => $value)
            {
                if($this->isValidValue($value))
                {
                    switch($column)
                    {
                        ...
                        case '_id':
                            if (!empty($value)) {
                                $elo = $elo->where($this->table . '.diary_id', '!=', $value);
                            }
                            break;
                        case 'status':
                            if (!empty($value)) {
                        ...
                        case 'keyword':
                            if (!empty($value)) {
                                $elo = $elo->where(function($elo) use ($value) {
                                    $elo->where($this->table . '.diary_name', 'LIKE', "%{$value}%")
    ...
    public function updateItem($params = [], $id = NULL) {
        ...
        //get diary item by conditions
        $_params = [
            'id' => $id,
        ];
        $diary = $this->selectItem($_params);
