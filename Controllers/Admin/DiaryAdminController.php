<?php namespace Foostart\Diary\Controllers\Admin;

/*
|-----------------------------------------------------------------------
| DiaryAdminController
|-----------------------------------------------------------------------
| @author: Kang
...
use Foostart\Diary\Validators\DiaryValidator;


class DiaryAdminController extends FooController {
    ...
    public function __construct(Request $request) {
        ...
        //models
        $this->obj_item = new diary(array('perPage' => 10));
        $this->obj_category = new Category();
        $this->obj_slideshow = new Slideshow();

        //validators
        $this->obj_validator = new DiaryValidator();

        //set language files
        $this->plang_admin = 'diary-admin';
        $this->plang_front = 'diary-front';

        //package name
        $this->package_name = 'package-diary';
        $this->package_base_name = 'diary';

        //root routers
        $this->root_router = 'diaries';
        ...
        $this->category_ref_name = 'admin/diaries';
        ...
        $this->breadcrumb_2['label'] = 'Diaries';
    ...
    public function index(Request $request) {
        ...
        if ($is_admin ) {
            ...
            return redirect()->route('diaries.list', ['user_id' => $user['user_id']]);
    ...
    /**
     * Processing data from Diary method: add new item, edit existing item
     * @return view edit page
     * @date 27/12/2017
     */
    ...
    public function config(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-diary.php'));
        $package_path = realpath(base_path('vendor/foostart/package-diary'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            file_put_contents($config_bakup.'/package-diary-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-diary.langs');
        $lang_paths = [];
        $package_path = realpath(base_path('vendor/foostart/package-diary'));

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/diary-admin.php'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name diary-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/diary-admin-'.date('YmdHis',time()).'.php', $content);
            }
